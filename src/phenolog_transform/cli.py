"""Command line interface for phenolog-transform."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """phenolog-transform CLI."""
    if version:
        from phenolog_transform import __version__
        typer.echo(f"phenolog-transform version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for phenolog-transform."""
    typer.echo("Downloading data for phenolog-transform...")
...
def transform(
    ...
    """Run the Koza transform for phenolog-transform."""
    typer.echo("Transforming data for phenolog-transform...")
